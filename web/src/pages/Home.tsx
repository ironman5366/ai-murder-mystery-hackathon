...
export default function Home() {
  ...
  const handleBackToGame = (answers: string[]) => {
    ...
    if (larry.length > 0) {
      ...
      if ((answers[0] != "Amateur Larry") || (answers[1] != "Getting back stolen treasure")) {
        let forcedMessage2 = ""
        forceTextResponseFromLarry(updatedActors[larryId], forcedMessage2)
      }
    }
    setEndGame(false);
    setPostGame(true);
  ...
  const forceTextResponseFromLarry = (actor: Actor, forcedMessage: string) => {
    const newMessage: LLMMessage = { role: "assistant", content: forcedMessage };
    sendChat([...actor.messages, newMessage], setActors, globalStory, sessionId, actor, setLoading);
  };
